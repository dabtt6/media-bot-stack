...
def build_queue():
    print("\n🟡 BUILD QUEUE")
    rows=c.execute("SELECT code FROM crawl GROUP BY code").fetchall()

    for (code,) in rows:
        ijav=c.execute("""
        SELECT source,torrent_url,size_mb,seeds,date_ts
        FROM crawl WHERE code=? AND source='ijav'
        ORDER BY date_ts DESC,size_mb DESC,seeds DESC LIMIT 1
        """,(code,)).fetchone()
        ...
        if ijav:
            ...
            one=c.execute("""
            SELECT source,torrent_url,size_mb,seeds,date_ts
            FROM crawl WHERE code=? AND source='onejav'
            LIMIT 1
            """,(code,)).fetchone()
            if not one: continue
            chosen=("onejav",)+one[1:]
        ...
        c.execute("""
        INSERT OR IGNORE INTO queuedqbit(code,chosen_source,torrent_url,size_mb,seeds,date_ts)
        VALUES(?,?,?,?,?,?)
        """,(code,)+chosen)
        conn.commit()

    print("Queued:",c.execute("SELECT COUNT(*) FROM queuedqbit").fetchone()[0])
